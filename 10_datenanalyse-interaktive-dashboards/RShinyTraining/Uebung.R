    ...
    output$Visualisierung <- plotly::renderPlotly({
        mitglieder %>%
            ...
            ggplot(aes(Geschlecht, fill = Geschlecht)) + # Hier legen wir fest, was auf der x-Achse angezeigt und wie die Graphik eingefärbt werden soll
              ...
              ggtitle(paste(input$ort,input$beitrittsjahr)) + # fügt einen Titel hinzu
              xlab("Geschlecht") + # beschriftet die x-Achse
              ylab("Anzahl") + # beschriftet die y-Achse
              geom_bar() # bestimmt die Art des Plots
    })

    ##### Übung 9: Fügt hier den Output für die Datentabelle der Mitglieder ein. Tipp: das Format lautet output$Outputname <- DT::renderDT({Datensatz})

    # Einfügen des Datums
    output$Datum <- renderText({
      paste("Auszug erstellt am", format(Sys.time(), "%d.%m.%Y"), ".")
    })
}
